/**
 * ISO 3166-1 alpha-2 -> numeric (3-digit, zero-padded) lookup.
 *
 * `world-atlas` TopoJSON keys countries by ISO numeric id (e.g. Netherlands "528").
 * Friendly alpha-2 codes in the data (`"NL"`) join to the map through this table.
 */
...
export function isValidAlpha2(code: string): boolean {
  return code.toUpperCase() in ALPHA2_TO_NUMERIC;
}
